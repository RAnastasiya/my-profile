...
import {Figures} from "../../../unit/figures";
...
class Tetris extends React.Component {
    ...
    componentDidMount() {
        // console.log("componentDidMount - ", this.state.sizeField);
        this.initField();
        this.initFigure();
        this.loop();
    ...
    componentDidUpdate(prevProps, prevState) {
        // console.log("componentDidUpdate - ", this.state.sizeField);
    }
    ...
    initField() {
        let w = this.state.sizeField.width;
        let h = this.state.sizeField.height;
        let cellSize = Number.isInteger(h/50) ? 50 : h / (Math.floor(h/50));
        let width = Math.floor(w/cellSize) * cellSize;

        let fieldWidth = Math.floor(width/(cellSize));
        let fieldHeight = Math.floor(h/cellSize);
        ...
        if (this.state.stepCounter % 3 === 0) {
            if (this.state.currentFigure) {
                if (!this.state.nextFigure) {
                    ...
                    console.log("randomFigure - ", randomFigure);
                    let figurePath = [...randomFigure.path];
                    if(this.state.fieldWidth > 10){
                        console.log("this.state.fieldWidth  - ", figurePath );
                        let randomCenter = Math.floor(Math.random() * (this.state.fieldWidth - 10));
                        console.log("randomCenter  - ", randomCenter );
                        console.log("randomFigure.path  - ", randomFigure.path );
                        figurePath = figurePath.map(item => [item[0] + randomCenter, item[1]]);

                    }
                    this.setState({
                        nextFigure: figurePath,
    ...
    render() {
        ...
        return (
            <div className="tetris" style={this.state.sizeField}>
                {this.state.field.map((row, i) =>
                    <Row key={i} row={row} cellSize={this.state.cellSize}/>
                )}
            </div>
        );
    }
